...
const DATABASE_URL =
  "postgresql://neondb_owner:npg_E5KhirvO0MHI@ep-fragrant-surf-a4ejben8-pooler.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require"
...
export async function query<T = any>(sqlQuery: string, params: any[] = []): Promise<T[]> {
  try {
    ...
    if (error instanceof Error && error.message.includes('NetworkError')) {
      console.error('NetworkError: Could not connect to Neon. Check your DATABASE_URL, Neon project status, and network.');
    }
    // Fallback to mock DB in preview or error
