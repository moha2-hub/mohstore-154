...
export async function deleteProduct(id: number) {
  try {
    ...
    if (!user) {
      console.error("deleteProduct: No user found (not authenticated)");
      return { success: false, message: "Not authenticated" };
    }
    if (user.role !== "admin") {
      console.error("deleteProduct: User is not admin", user);
      return { success: false, message: "Unauthorized: Only admin can delete products" };
    }
    await query("DELETE FROM products WHERE id = $1", [id]);
    return { success: true };
  } catch (error) {
    console.error("Delete product error:", error, "Product ID:", id);
    return { success: false, message: `Failed to delete product: ${error && error.message ? error.message : error}` };
  }
}
